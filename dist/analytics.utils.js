...
export const calculateFocusScore = (duration, distractionCount) => {
    if (duration === 0)
        return 100;
...
export const generateDistractionHeatmap = (distractions, startTime, duration, intervalMinutes = 5) => {
    const intervals = Math.ceil(duration / intervalMinutes);
    const heatmap = new Array(intervals).fill(0);
...
export const getDistractionIntensity = (count) => {
    if (count === 0)
        return 'low';
    if (count === 1)
        return 'medium';
...
export const generateSuggestions = (duration, distractions, heatmap) => {
    const suggestions = [];
